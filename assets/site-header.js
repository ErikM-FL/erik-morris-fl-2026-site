...
(function(){
  ...
  function build(){
    ...
    [ {label:'Español',href:root+'/index-es.html'}, {label:'Kreyòl Ayisyen',href:root+'/index-ht.html'}, {label:'中文（简体）',href:root+'/index-zh.html'}, {label:'Tiếng Việt',href:root+'/index-vi.html'}, {label:'한국어',href:root+'/index-ko.html'}, {label:'Português',href:root+'/index-pt.html'} ]
      .forEach(i=> list.appendChild(el('li',{}, `<a href="${i.href}">${i.label}</a>`)) ); grp.append(list); menu.append(grp);
    lang.append(btn,menu);
    btn.addEventListener('click',()=>{ const open=btn.getAttribute('aria-expanded')==='true'; btn.setAttribute('aria-expanded', String(!open)); menu.setAttribute('aria-hidden', String(open)); if(!open){ loadGT(initGT); } });
