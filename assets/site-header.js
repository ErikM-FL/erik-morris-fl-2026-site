...
(function(){
  ...
  const NAV_LINKS = [
    ...
    { label: "How to Write-In", href: "/pages/how-to-write-in.html" }
  ];
  const LANG_MENU = [
  ...
  function el(t,a={},h){ const e=document.createElement(t); for(const k in a){ if(k==='class') e.className=a[k]; else e.setAttribute(k,a[k]); } if(h!=null) e.innerHTML=h; return e; }

  function ensureStyles(){ if(!document.querySelector('link[href$="/assets/site-header.css"]')){ const l=el('link',{rel:'stylesheet',href:'/assets/site-header.css'}); (document.head||document.body).appendChild(l);} }

  function loadGT(cb){ if(window.google && window.google.translate){ cb(); return; } const s=document.createElement('script'); s.src='//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit'; window.googleTranslateElementInit=function(){ cb(); }; document.body.appendChild(s); }
  ...
  function build(){ ensureStyles(); const holder=document.getElementById('site-header'); if(!holder) return; const H=el('header',{class:'site-header'}); const inner=el('div',{class:'site-header__inner'}); const top=el('div',{class:'site-header__top'}); const bottom=el('nav',{class:'site-header__bottom site-header__nav','aria-label':'Primary'});
    const tg=el('div',{class:'site-header__tagline'}); tg.textContent=TAGLINE_TEXT; const images=el('div',{class:'site-header__images'}); const img1=el('img',{class:'site-header__img',src:'/assets/EMtest.png',alt:''}); const img2=el('img',{class:'site-header__img',src:'/assets/write-in-ballot.gif',alt:'Write-in ballot'}); img1.onerror=()=>{img1.style.display='none'}; img2.onerror=()=>{img2.style.display='none'}; images.append(img1,img2);
    const sel=el('select',{class:'site-header__lang-select','aria-label':'Languages'}); sel.appendChild(el('option',{value:'',selected:true,hidden:true},'Languages')); LANG_MENU.forEach(i=>{ if(i.type==='sep'){ const o=el('option',{},'────────────'); o.disabled=true; sel.appendChild(o);} else if(i.type==='gt'){ sel.appendChild(el('option',{value:'__gt__'},i.label)); } else { sel.appendChild(el('option',{value:i.path},i.label)); } }); sel.addEventListener('change',e=>{ const v=e.target.value; if(!v) return; if(v==='__gt__'){ openGT(); sel.value=''; return;} location.href=v; });
    const langWrap=el('div'); langWrap.appendChild(sel);
    top.append(tg,images,langWrap); NAV_LINKS.forEach(({label,href})=> bottom.appendChild(el('a',{href},label)) ); inner.append(top,bottom); H.appendChild(inner); holder.replaceWith(H); }
